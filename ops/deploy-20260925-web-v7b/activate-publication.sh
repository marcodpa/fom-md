...
release=/var/www/fom-web/releases/20260925-web-v7b
backup=/var/backups/fom-web/20260925-web-v7b-$(date +%Y%m%dT%H%M%S)-$$
# Accept the config left by the 2026-09-25 v7 publication (releases/20260925-web-v7) or,
# if that one never ran, by the 2026-09-22 republication (releases/20260921-public-original-v2).
current=$(sha256sum "$config" | cut -d ' ' -f1)
test "$current" = c7fc3c674ebfd7ab91fd5b88bf14231f757c4b13c270abd48526560c00cfbbbd || test "$current" = 2148712f66ec3f2828ea831d5904faa4419763200e756d8a12b68f21a9fb17a7 || { echo 'Nginx cambió desde la preparación; revisar antes de publicar.' >&2; exit 1; }
test ! -e "$backup"
cd "$stage"
